...
public final class MySQLUtils {
	...
	public static java.sql.Time toTime(int value) {
		final int s = value % 100; value /= 100;
		final int m = value % 100; 
		final int h = value / 100;
		return new java.sql.Time(h*3600*1000+m*60000+s*1000);
	...
	public static int getLengthEncodedInteger(byte[] data) {
		int re=0;
		if((data[0]&0x00ff)<0x00fb){
			re|=(data[0]&0x00ff);
			return re;
		}
		if(data[0]==0xfc){
